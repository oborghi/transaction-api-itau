...
class AuthorizeTransactionUseCase(
    ...
    fun execute(transactionId: String, request: TransactionRequest): TransactionAuthorizationResponse {
        ...
        // Create X-Ray subsegment for the transaction authorization flow
        val subsegment = AWSXRay.beginSubsegment("transaction.authorize")
        try {
            subsegment?.putAnnotation("transaction_id", transactionId)
            subsegment?.putAnnotation("account_id", request.account_id)
            subsegment?.putAnnotation("transaction_type", request.type)
            subsegment?.putAnnotation("amount", request.amount.value)
            subsegment?.putAnnotation("currency", request.amount.currency)

            val result = metricsPort.recordAuthorizationLatency {
                ...
                subsegment?.putAnnotation("transaction_status", response.transaction.status)
                subsegment?.putAnnotation("balance_after", response.account.balance.value)
            ...
            subsegment?.putAnnotation("error", e.message ?: "Unknown error")
            throw e
        } finally {
            subsegment?.end()
        }
    ...
    fun fallback(transactionId: String, request: TransactionRequest, ex: Exception): TransactionAuthorizationResponse {
        ...
        // Add X-Ray subsegment for circuit breaker fallback
        val subsegment = AWSXRay.beginSubsegment("circuitbreaker.fallback")
        try {
            subsegment?.putAnnotation("transaction_id", transactionId)
            subsegment?.putAnnotation("account_id", request.account_id)
            subsegment?.putAnnotation("error", ex.message ?: "Circuit breaker opened")
            subsegment?.setError(true)
            throw ex
        } finally {
            subsegment?.end()
